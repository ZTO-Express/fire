...
class FireILoop(
  ...
  // remote environment
  private lazy val (remoteBenv: ScalaShellEnvironment,
  ...
  val (
    ...
    table: StreamTableEnvironment
    ) = {
    PropUtils.setProperty(FireFrameworkConf.FIRE_ENV_LOCAL, "true")
    Test.main(null)
    val scalaSenv = Test.getFire
    val scalaSTEnv = Test.getStreamTableEnv
    (scalaSenv, scalaSTEnv)
  }
