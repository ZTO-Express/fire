...
public class RocksDBStateBackend extends AbstractManagedMemoryStateBackend
    ...
    // TODO: ------------ start：二次开发代码 --------------- //
    /**
     * State disk choose policy
     */
    private static final String FLINK_STATE_DISK_CHOOSE_POLICY_ROUND_ROBIN = "ROUND_ROBIN";

    /**
     * Default state disk choose policy
     */
    private static final String FLINK_STATE_DISK_CHOOSE_POLICY_DEFAULT = "DEFAULT";
     ...
     */
    private String stateDiskPolicy;
    ...
    // 状态本地磁盘路径选取策略：default/round_robin
    private final static String STATE_CHOOSE_DISK_POLICY = "flink.state.choose.disk.policy";
    ...
    private void lazyInitializeForJob(
        ...
        if (localRocksDbDirectories == null) {
            ...
            LOG.info("initializedDbBasePaths.size:" + initializedDbBasePaths.length);
            for (File file : initializedDbBasePaths) {
                LOG.info("initializedDbBasePaths:" + file.getPath());
            }
        } else {
    ...
    private File getNextStoragePath() {
        ...
        if (isRoundRobin()) {
            try {
                ...
                ni = new Random().nextInt(initializedDbBasePaths.length);
                LOG.error("基于zookeeper的本地状态磁盘路径选择发生异常，请在commons.properties文件中指定以下参数恢复到flink默认的选择策略：flink.state.choose.disk.policy=default", e);
            }
        } else {
            ni = nextDirectory + 1;
        ...
        LOG.info("Next state file storage path is: " + initializedDbBasePaths[ni].getPath());
        // TODO: ------------ end：二次开发代码 --------------- //
