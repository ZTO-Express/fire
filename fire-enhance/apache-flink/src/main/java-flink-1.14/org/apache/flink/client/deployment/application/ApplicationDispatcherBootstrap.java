...
public class ApplicationDispatcherBootstrap implements DispatcherBootstrap {
    ...
    private CompletableFuture<Void> fixJobIdAndRunApplicationAsync(
        ...
        if (!configuredJobId.isPresent()) {
            String haClusterId = configuration.getString(HighAvailabilityOptions.HA_CLUSTER_ID);
            String[] splits = (haClusterId != null ? haClusterId : "").split("_");
            if (splits != null && splits.length == 3) {
                configuration.set(
                        PipelineOptionsInternal.PIPELINE_FIXED_JOB_ID, new JobID(Long.valueOf(splits[1]), Long.valueOf(splits[2])).toHexString());
            } else {
                configuration.set(
