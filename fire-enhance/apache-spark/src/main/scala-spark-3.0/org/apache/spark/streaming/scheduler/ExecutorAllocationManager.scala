...
private[streaming] class ExecutorAllocationManager(
  ...
  private val downStep = conf.get(STREAMING_DYN_ALLOCATION_EXECUTOR_DOWN_STEP)
  ...
  private def manageAllocation(): Unit = synchronized {
    ...
    if (batchProcTimeCount > 0) {
      ...
      if (ratio >= scalingUpRatio) {
        ...
        // TODO: ------------ start：二次开发代码 --------------- //
        val numNewExecutors = math.max(math.round(ratio).toInt, upStep)
        // TODO: ------------ end：二次开发代码 --------------- //
        requestExecutors(numNewExecutors)
      } else if (ratio <= scalingDownRatio) {
        logDebug("Killing executors")
        // TODO: ------------ start：二次开发代码 --------------- //
        (1 to downStep).foreach(_ => killExecutor())
        // TODO: ------------ end：二次开发代码 --------------- //
      }
    }
    batchProcTimeSum = 0
