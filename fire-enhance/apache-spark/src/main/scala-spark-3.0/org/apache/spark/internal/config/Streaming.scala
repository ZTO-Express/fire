...
private[spark] object Streaming {
  ...
  // TODO: ------------ start：二次开发代码 --------------- //
  private[spark] val STREAMING_DYN_ALLOCATION_EXECUTOR_UP_STEP =
    ConfigBuilder("spark.streaming.dynamicAllocation.upStepSize")
      .version("3.0.0")
      .intConf
      .checkValue(_ > 0, "The number of executors dynamically applied for each time.")
      .createWithDefault(1)

  private[spark] val STREAMING_DYN_ALLOCATION_EXECUTOR_DOWN_STEP =
    ConfigBuilder("spark.streaming.dynamicAllocation.downStepSize")
      .version("3.0.0")
      .intConf
      .checkValue(_ > 0, "The number of executors dynamically released each time.")
      .createWithDefault(1)
  // TODO: ------------ end：二次开发代码 --------------- //
}
