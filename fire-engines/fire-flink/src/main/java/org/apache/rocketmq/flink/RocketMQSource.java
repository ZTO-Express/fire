...
import org.apache.rocketmq.flink.serialization.TagKeyValueDeserializationSchema;
...
public class RocketMQSource<OUT> extends RichParallelSourceFunction<OUT>
    ...
    public RocketMQSource(TagKeyValueDeserializationSchema<OUT> schema, Properties props) {
        this.schema = schema;
        this.props = props;
    }
                            ...
                            switch (pullResult.getPullStatus()) {
                                case FOUND:
                                    ...
                                    for (MessageExt msg : messages) {
                                        byte[] tags = msg.getTags() != null ? msg.getTags().getBytes(StandardCharsets.UTF_8) : null;
                                        byte[] key = msg.getKeys() != null ? msg.getKeys().getBytes(StandardCharsets.UTF_8) : null;
                                        byte[] value = msg.getBody();
                                        OUT data = schema.deserializeTagKeyAndValue(tags, key, value);
